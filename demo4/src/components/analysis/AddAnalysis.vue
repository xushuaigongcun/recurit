...
<el-form  :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm" >
  ...
  <el-form-item label="请选择项目" prop="programmName">
    <el-select v-model="ruleForm.programmId" placeholder="请选择项目" @change="getCustorm">
  ...
  <el-form-item>
<!-- 弹出框Form -->
<el-button type="primary" @click="dialogFormVisible = true">添加职位需求</el-button>
...
<el-dialog title="需求信息" :visible.sync="dialogFormVisible">
  <el-form :model="form" ref="form" :rules="dialogRule" label-width="formLabelWidth" >
    ...
    <el-form-item label="招聘人数" :label-width="formLabelWidth" required prop="requireNumber">
      <el-input v-model.number="form.requireNumber" autocomplete="off"></el-input>
    </el-form-item>
      </el-col>
    </el-row>
  <el-row>
    <el-form-item label="工作描述" :label-width="formLabelWidth" prop="jobDesc">
      <el-input v-model="form.jobDesc" autocomplete="off"></el-input>
       ...
       <el-form-item label="负责人" :label-width="formLabelWidth" required prop="contact">
  ...
  <el-row>
    ...
    <el-select v-model="form.recuritIds" placeholder="请选择招聘人" multiple>
      <el-option :label="programm.programmName" :value="programm.programmId" v-for="programm  in programms" :key = "programm.programmId" ></el-option>
    </el-select>
  </el-form-item>
...
<template v-if="tableDataValue.length>0">
  <el-table
    ...
    <el-table-column
      ...
      prop="endTime"
      label="结束时间"
      width="120" value-format="timestamp">
    </el-table-column>
    <el-table-column
      fixed="right"
      label="操作"
...
<script>
  export default {
    ...
    methods: {
      ...
      sendForm(){
        const{programmId,custormId,custormName,deptId,deptName,startTime,endTime,requireNumber,demandDesc} =this.ruleForm;
        var tableDataValue = [];
        ...
        console.log(tableDataValue); 
        this.$axios.post("/recurit",{programmId,custormId,custormName,deptId,deptName,startTime,endTime,requireNumber,demandDesc,tableDataValue})
      ...
      add(){
        this.addForm('form');
        const { jobName,address,jobDesc,startTime,endTime,skill,requireNumber,recuritIds,contact} =this.form;
        var table = {jobName: '',
          address: '',
          jobId:'',
          jobDesc: '',
